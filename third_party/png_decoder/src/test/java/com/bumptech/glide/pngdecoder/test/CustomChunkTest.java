...
public class CustomChunkTest {
  ...
  private void readWithouthFactory() {
    ...
    TestCase.assertEquals("Second chunk should be  prOp", "prOp", pngr.getChunksList().getChunks()
        .get(1).getRaw().id);
    TestCase.assertEquals("Next chunk should be  pHYs", "pHYs", pngr.getChunksList().getChunks()
        .get(2).getRaw().id);
