...
public class SampleImgDiff {
   ...
   * ignores alpha
   */
  private static int[] computeDiffLine(ImageLineInt l1, ImageLineInt l2) {
    double maxDif = -1;
    int maxDifCol = -1;
    double dif;
    ...
    for (int col = 0; col < l1.imgInfo.cols; col++) {
      dif =
          Math.abs(scanline1[col * channels] - scanline2[col * channels])
  ...
  public static void showDiff(String f1, String f2) {
    ...
    for (int row = 0; row < rows; row++) {
      ImageLineInt l1 = (ImageLineInt) i1.readRow(row);
      ImageLineInt l2 = (ImageLineInt) i2.readRow(row);
